...
def fetch_rss_feed(url):
    ...
    for item in items:
        article = {
            ...
            'published': item.find('pubDate').text if item.find('pubDate') is not None else 'Unknown date',
...
def fetch_news(config_path):
    ...
    for source in config['sources']:
        try:
            if 'url' in source:
                articles.extend(fetch_rss_feed(source['url']))
            else:
                print(f"Skipping source {source.get('name', 'Unknown')}: No URL provided")
        except Exception as e:
            print(f"Error fetching {source.get('name', 'Unknown')}: {e}")
    return articles
...
for article in articles:
    ...
    if not c.fetchone():
        ...
        """, (
            ...
            article["description"], summary, json.dumps(bullet_points), 
            json.dumps(market_implications), "neutral", False, importance_score
        ))
