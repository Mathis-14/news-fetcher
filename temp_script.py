...
for article in articles:
    ...
    if not c.fetchone():
        ...
        c.execute(
            "INSERT INTO articles (title, url, source, published, description, summary, bullet_points, market_implications, political_stance, is_fake_news, importance_score) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                article["title"],
                article["url"],
                article["source"],
